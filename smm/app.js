...
function initTableUpload() {
  ...
  function renderPreview(meta) {
    if (!meta || !(meta.columns || []).length) { prev.innerHTML = ''; return; }
  ...
  /* У карточки CSV теперь реальная форма, а не заглушка. */
  const csvCard = sources.querySelector('.pick-card[data-src="csv"]');
  if (csvCard) {
    const need = csvCard.querySelector('.pick-need');
    if (need) need.remove();
    const soon = csvCard.querySelector('.pill.soon');
  ...
  sources.addEventListener('click', async e => {
    ...
    if (!card) return;
    const isCsv = card.dataset.src === 'csv';
    box.hidden = !isCsv;
    if (!isCsv) return;
    ...
    if (!loadedSaved && PID) {
      loadedSaved = true;
      try {
        const saved = await api.previewTable(PID);
        if (saved && (saved.columns || []).length) renderPreview(saved);
