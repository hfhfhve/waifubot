...
const LIB_PAGE_TYPES = [
  ...
  { id: 'status',   icon: 'clock',  name: 'Статус с датой',
    ...
    design: 'Большая плашка с датой, таблица состояний, история изменени��' },
...
function initTableUpload() {
  ...
  function renderPreview(meta, into) {
    ...
    const head = `
      <div class="note info mt-3"><div class="note-body">
        Это предпросмотр файла: строк в проекте пока нет.
        Выберите колонку-ключ и нажмите «Загрузить строки» — только после этого
        строки появятся в проекте и генерацию можно будет запустить.
      </div></div>
...
/* --------------------------------------------------------------------------
   ...
   Главное: часть проверок Google не использует вообще — водность,
   индексы читаемости, meta keywords. Они выключены на бэкенде и
   показаны здесь с пометкой, чтобы было видно почему.
   Тошнота удалена из проекта целиком и больше не приходит с бэкенда.
   -------------------------------------------------------------------------- */
...
function renderGateConfig() {
  ...
  if (note) {
    note.innerHTML =
      ...
      'читаемости нет, а тег keywords игнорируется с 2009 года. От��лючённые проверки ' +
